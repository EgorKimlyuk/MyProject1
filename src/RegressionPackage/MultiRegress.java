...
public class MultiRegress {
     ...
     public static double[] regress(ArrayList<Point3D> list){
        ...
        Jama.Matrix x = new Matrix(new double[][]{{0.0},{0.0},{0.0}});
         try {
            x = A.solve(bb);
         }catch (Exception e){
             if(SettingsClass.isIsThisPicture()){
                x.set(0,0,255);
                x.set(1,0,255);
                x.set(2,0,255);
             }
        ...
        mas[0] = x.get(0,0);//возвращает коэфициент a1
        mas[1] = x.get(1,0);// возвращает коэфициент а2
        mas[2] = x.get(2,0);//возвращает свободный член с
